...
def get_weather(bot: Bot, lat: float, lon: float, city_name: str = None):
    ...
    weather_data = requests.get(
        f"https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&exclude=minutely,hourly,alerts&appid={config.open_weather_token}&units=metric&lang=ru"
    ).json()
    ...
    for i in range(3):
        ...
        weather_desc = f"{EMOJI[weather_desc]} " if weather_desc in EMOJI else ""
        answer += (
            f"<b>Прогноз на {datetime.utcfromtimestamp(weather_data['daily'][i]['dt']).strftime('%e %B %Y').strip()}:</b>\n"
